 ...
 */
public class CustomHarmonicAccelerationModel implements AccelerationModel {
     ...
     * fundamental period)
     */
    public CustomHarmonicAccelerationModel(final String prefix, final AbsoluteDate referenceDate,
    ...
    public double signedAmplitude(final SpacecraftState state,
        ...
        return parameters[0] * FastMath.sin(dt * omega) + parameters[1] * FastMath.cos(dt * omega);
    }
    ...
    public <T extends CalculusFieldElement<T>> T signedAmplitude(final FieldSpacecraftState<T> state,
        ...
        return parameters[0].multiply(dt.multiply(omega).sin()).add(parameters[1].multiply(dt.multiply(omega).cos()));
    }
